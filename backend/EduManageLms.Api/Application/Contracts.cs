...
public sealed record CourseGradeDto(string CourseId,string CourseCode,string CourseName,int Credits,string ClassSectionCode,string LecturerName,IReadOnlyCollection<GradeComponentDto> Scores,double FinalScore,string LetterGrade,double GradePoint,bool Passed,DateTime? PublishedAt);
...
public interface IAuthService{Task<LoginResponse> LoginAsync(LoginRequest request,string ip,string userAgent,CancellationToken ct);Task<LoginResponse> RefreshAsync(string token,string userAgent,CancellationToken ct);Task RevokeAsync(string token,CancellationToken ct);Task RevokeAllAsync(string userId,CancellationToken ct);Task ChangePasswordAsync(string userId,string currentPassword,string newPassword,CancellationToken ct);}
public interface IDashboardService{Task<DashboardDto> AdminAsync(CancellationToken ct);Task<DashboardDto> LecturerAsync(string lecturerCode,CancellationToken ct);Task<DashboardDto> StudentAsync(string studentCode,CancellationToken ct);}
public interface IStudentAnalyticsService{Task<IReadOnlyCollection<CourseGradeDto>> GetGradesAsync(string studentCode,string? year,string? semester,CancellationToken ct);Task<StudentGpaDto> GetCumulativeGpaAsync(string studentCode,CancellationToken ct);Task<IReadOnlyCollection<CloResultDto>> GetCloAsync(string studentCode,CancellationToken ct);}
public interface IGradebookService{Task<GradebookDto> GetAsync(string lecturerCode,string? sectionId,CancellationToken ct);Task UpdateAsync(string lecturerCode,string sectionId,GradeUpdateRequest request,string userId,CancellationToken ct);}
public interface IAdminResourceService{Task<PagedResult<Dictionary<string,object?>>> ListAsync(string resource,string? search,int page,int size,CancellationToken ct);Task<Dictionary<string,object?>> CreateAsync(string resource,Dictionary<string,object?> body,CancellationToken ct);Task<Dictionary<string,object?>> UpdateAsync(string resource,string id,Dictionary<string,object?> body,CancellationToken ct);Task DeleteAsync(string resource,string id,CancellationToken ct);}
public interface IBackupService{Task<Dictionary<string,object>> CreateAsync(string userId,CancellationToken ct);Task<IReadOnlyCollection<Dictionary<string,object?>>> ListAsync(CancellationToken ct);Task RestoreAsync(string id,string userId,string confirmation,CancellationToken ct);}
