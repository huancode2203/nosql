...
@Component({
  ...
  template: `
    <app-page-header title="Sao lưu và phục hồi" subtitle="Quản lý bản sao lưu MongoDB bằng mongodump và mongorestore.">
      @if(auth.hasPermission('admin.backups.manage')){<input #backupFile type="file" accept=".zip" hidden (change)="upload($any($event.target).files?.[0]); backupFile.value=''"/>
...
export class BackupsComponent implements OnInit {
  ...
  download(item:BackupItem){this.api.getBlob(`/admin/backups/${item.id}/download`).subscribe({next:blob=>{const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`${item.fileName}.zip`;a.click();URL.revokeObjectURL(url);},error:e=>this.fail(e,'Không thể tải bản sao lưu')});}
